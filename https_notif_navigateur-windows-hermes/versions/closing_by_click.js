...
function appendNotifHtml() {
 ...
 `);
	...
	// Добавляем стиль для отключения transition на top
...
function initDraggable() {
	...
	let lastMoveTime = 0;
	let initialMouseY = 0; // Начальная позиция курсора

	// Сразу устанавливаем fixed позиционирование и отключаем transition
	...
	const dragStart = (e) => {
		...
		if (e.target.closest('.history-button')) {
			...
			// Запоминаем текущую позицию контейнера и курсора
	...
	const dragEnd = (e) => {
		...
		if (!wasRealDrag &&
			...
			// Сохраняем текущее положение перед открытием
			const prevTop = container.style.top;

			showCallHistoryModal();

			// Восстанавливаем положение после открытия
			container.style.transition = 'none';
	...
	const drag = (e) => {
		...
		// Полностью отключаем плавность для более точного позиционирования
		...
		// Убираем плавность полностью - сразу используем точную позицию
		currentY = boundedY;

		// Обновляем позицию кнопки без плавности (важно для предотвращения "прыжков")
		container.style.transition = 'none';
...
if (callButton) {
	callButton.addEventListener('click', function (e) {
		// предотвращаем нежелательные переходы фокуса/drag'а
		e.preventDefault();
		e.stopPropagation();
